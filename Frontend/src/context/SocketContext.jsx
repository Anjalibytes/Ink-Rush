...
export function SocketProvider({ children }) {
  ...
  // Visual feedback triggers (UI-only, no game logic)
  const [scoreFx, setScoreFx] = useState(null); // { id, points } when *you* guess right
  const [wrongGuessKey, setWrongGuessKey] = useState(0); // bumps on each of your misses
  ...
  useEffect(() => {
    ...
    socket.on("player_joined", ({ player, players }) => {
      ...
      setChatMessages((prev) => [
        ...prev,
        { system: true, text: `${player.username} hopped in — say hi!` }
      ]);
    });
    ...
    socket.on("player_left", ({ playerId, username, room }) => {
      ...
      setChatMessages((prev) => [
        ...prev,
        { system: true, text: `${username} ducked out. Pencils down for them.` }
      ]);
    });
    ...
    socket.on("game_started", ({ room }) => {
      setScreen("GAME_PLAY");
      setRoomState(room);
      setChatMessages([{ system: true, text: "Pencils up! Let the doodling begin." }]);
    });
    ...
    socket.on("word_chosen", ({ drawerId, category, length, room }) => {
      ...
      // Start the countdown ring full instead of showing last round's leftover value
      setTimeLeft(room.remainingTime || room.settings?.drawTime || 0);
    });
    ...
    socket.on("chat_message", (message) => {
      ...
      // Your own guess came back as a plain chat line => it missed
      if (message?.isGuess && message.playerId === myPlayerId) {
        setWrongGuessKey((k) => k + 1);
        sound.miss();
      }
    });
    ...
    socket.on("guess_result", ({ correct, points, word }) => {
      if (correct) {
        ...
        setScoreFx({ id: Date.now(), points });
        sound.ding();
        setChatMessages((prev) => [
          ...prev,
          { system: true, guessCorrect: true, text: `Nailed it! +${points} points` }
        ]);
        if (word) {
          setChosenWordReveal(word);
        }
